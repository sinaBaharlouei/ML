...
def find_distance(vec1, vec2, dist_type):
    ...
    for ind in range(length):
        if dist_type == "Euclidean":
            ...
            dist += abs(int(vec1[ind]) - int(vec2[ind]))
        else:
...
def calculate(percentage, distance_type):
    ...
    for item in test_data:
        ...
        classes_score = [[2, 0.0], [4, 0.0]]

        for counter in range(k):
            ind = sorted_distance_vector[counter][0]
            for elem in classes_score:
                if float(elem[0]) == float(sample_train_target[ind]):
                    if float(sorted_distance_vector[counter][1]) != 0.0:
                        elem[1] += float(1) / float(sorted_distance_vector[counter][1])
                    else:
                        elem[1] += 2.0

        sorted_class_vector = sorted(classes_score, key=lambda x: x[1])
        test_data_predicted_target.append(sorted_class_vector[1][0])

    for i1 in range(len(test_data_predicted_target)):
        if int(test_data_predicted_target[i1]) == int(test_data_actual_target[i1]):
            correct_answers += 1
        else:
            wrong_answers += 1
...
with open('breast-cancer-wisconsin.csv', 'rb') as f:
    reader = csv.reader(f)
...
# missing values
column_based_vector = zip(*your_list)
mode_list = []
for item in column_based_vector:
    mode_list.append(mode(item))

for item in your_list:
    for i in range(number_of_features - 2):
        if item[i] == '?':
            item[i] = mode_list[i]
